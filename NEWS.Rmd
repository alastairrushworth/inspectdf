...
- __Important change:__ the Boolean option `show_plot` has been removed from all `inspect_*()` functions.  To generate visualisations of data frame summaries, use `show_plot(inspect_*())` or using the pipe  `inspect_*() %>% show_plot()`.
- `show_plot()` improvement that nudges points that might otherwise have coincided for dataframe comparisons of imbalance (eg. with `inspect_imb(df1, df2) %>% show_plot()`)  
