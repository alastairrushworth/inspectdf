...
- Fixed [#45](https://github.com/alastairrushworth/inspectdf/issues/45), partial argument matching warning in `format_size()`. Changed `unit = "auto"` to `units = "auto"` in call to `format()`. Thanks to [@salim-b](https://github.com/salim-b) for the report.
...
- `inspect_cor()` can be filtered prior to plotting, for example `inspect_cor(starwars) %>% filter(abs(corr) > 0.2) %>% show_plot()`.  Thanks to [Roel Verbelen](https://github.com/RoelVerbelen) for the [suggestion](https://github.com/alastairrushworth/inspectdf/issues/24)
