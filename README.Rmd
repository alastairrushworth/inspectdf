...
A point and whiskers plot is printed when using the `show_plot()` function:
...
The argument `high_cardinality` in the `show_plot()` function can be used to bundle together categories that occur only a small number of times.  For example, to combine categories only occuring once, use:
