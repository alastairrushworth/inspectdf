...
report_association <- function(df, df2 = NULL, top_n = NULL, type = "df"){
  ...
  if(is.null(df2)){
    ...
    s1 <- report_association(df,   top_n = top_n, type = type) %>% dplyr::rename(k_tau_1 = association) %>% select(-col_1, -col_2)
    s2 <- report_association(df2,  top_n = top_n, type = type) %>% dplyr::rename(k_tau_2 = association) %>% select(-col_1, -col_2)
