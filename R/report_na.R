...
report_na <- function(df1, df2 = NULL, top = NULL, show_plot = F){
  ...
  if(is.null(df2)){
    ...
    s1 <- report_na(df1,  top = top, show_plot = F) %>% dplyr::rename(count_na_1 = count_na, percent_na_1 = percent_na)
    s2 <- report_na(df2, top = top, show_plot = F) %>% dplyr::rename(count_na_2 = count_na, percent_na_2 = percent_na)
