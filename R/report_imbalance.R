...
report_imbalance <- function(df, df2 = NULL, top_n = NULL, type = "df"){
  ...
  if(is.null(df2)){
    ...
    s1 <- report_imbalance(df,  top_n = top_n, type = type) %>% dplyr::rename(value_1 = value, percent_1 = percent_in_col)
    s2 <- report_imbalance(df2, top_n = top_n, type = type) %>% dplyr::rename(value_2 = value, percent_2 = percent_in_col)
