...
test_that("inspect_num plots", {
  expect_doppelganger("Inspect-num-starwars", starwars %>% inspect_num %>% plot)
  expect_doppelganger("Inspect-num-storms",   storms %>%   inspect_num %>% plot)
  expect_doppelganger("Inspect-num-tech",   tech %>%   inspect_num %>% plot)
})

test_that("inspect_num plot paired", {
  expect_doppelganger("Inspect-num-paired-starwars", starwars %>% inspect_num(x2) %>% plot)
  expect_doppelganger("Inspect-num-paired-storms",   storms %>%   inspect_num(y2) %>% plot)
})

test_that("inspect_num plot suppress labels", {
  expect_doppelganger("Inspect-num-no-lab-starwars", starwars %>% inspect_num %>% plot(text_labels = FALSE))
  expect_doppelganger("Inspect-num-no-lab-tech", tech %>% inspect_num %>% plot(text_labels = FALSE))
...
test_that("inspect_num plot color palette", {
  expect_doppelganger("Inspect-num-palette-starwars", starwars %>% inspect_num() %>% plot(col_palette = 1))
