...
test_that("Plot output works", {
  set.seed(10)
  expect_is(inspect_types(mtcars, mtcars %>% dplyr::sample_n(100, replace = T), show_plot = T), "data.frame")
  expect_is(inspect_types(band_instruments, band_instruments %>% dplyr::sample_n(100, replace = T), show_plot = T) , "data.frame")
  expect_is(inspect_types(starwars, starwars %>% dplyr::sample_n(100, replace = T), show_plot = T), "data.frame")
  expect_is(inspect_types(storms, storms %>% dplyr::sample_n(100, replace = T), show_plot = T), "data.frame")
  expect_is(inspect_types(airquality, airquality%>% dplyr::sample_n(100, replace = T), show_plot = T), "data.frame")
})