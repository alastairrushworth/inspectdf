...
test_that("inspect_mem plots", {
  expect_doppelganger("Inspect-mem-starwars", starwars %>% inspect_mem %>% plot)
  expect_doppelganger("Inspect-mem-storms",   storms %>%   inspect_mem %>% plot)
  expect_doppelganger("Inspect-mem-tech",   tech %>%   inspect_mem %>% plot)
})

test_that("inspect_mem plot paired", {
  expect_doppelganger("Inspect-mem-paired-starwars", starwars %>% inspect_mem(x2) %>% plot)
  expect_doppelganger("Inspect-mem-paired-storms",   storms %>%   inspect_mem(y2) %>% plot)
  expect_doppelganger("Inspect-mem-paired-tech",   tech %>%   inspect_mem(z2) %>% plot)
})

test_that("inspect_mem plot suppress labels", {
  expect_doppelganger("Inspect-mem-no-lab-starwars", starwars %>% inspect_mem %>% plot(text_labels = FALSE))
  expect_doppelganger("Inspect-mem-no-lab-tech", tech %>% inspect_mem %>% plot(text_labels = FALSE))
...
test_that("inspect_mem plot color palette", {
  expect_doppelganger("Inspect-mem-palette-starwars", starwars %>% inspect_mem() %>% plot(col_palette = 1))
