...
test_that("inspect_cor plots", {
  expect_doppelganger("Inspect-cor-starwars", starwars %>% inspect_cor %>% plot)
  expect_doppelganger("Inspect-cor-storms",   storms %>%   inspect_cor %>% plot)
  expect_doppelganger("Inspect-cor-tech",   tech %>%   inspect_cor %>% plot)
})

test_that("inspect_cor spearman & kendall", {
  expect_doppelganger("Inspect-cor-kendall-starwars", starwars %>% inspect_cor(method = 'kendall') %>% plot)
  expect_doppelganger("Inspect-cor-spearman-storms",   storms %>%   inspect_cor(method = 'spearm') %>% plot)
  expect_doppelganger("Inspect-cor-kendall-tech",   tech %>%   inspect_cor(method = 'ken') %>% plot)
})

test_that("inspect_cor plot paired", {
  expect_doppelganger("Inspect-cor-paired-starwars", starwars %>% inspect_cor(x2) %>% plot)
  expect_doppelganger("Inspect-cor-paired-storms",   storms %>%   inspect_cor(y2) %>% plot)
})

test_that("inspect_cor plot suppress labels", {
  expect_doppelganger("Inspect-cor-no-lab-starwars", starwars %>% inspect_cor %>% plot(text_labels = FALSE))
  expect_doppelganger("Inspect-cor-no-lab-tech", tech %>% inspect_cor %>% plot(text_labels = FALSE))
...
test_that("inspect_cor plot grouped", {
  expect_doppelganger("Inspect-cor-grouped-starwars", starwars %>% dplyr::group_by(gender) %>% inspect_cor %>% plot)
  expect_doppelganger("Inspect-cor-grouped-tech",   tech %>% dplyr::group_by(year) %>%  inspect_cor %>% plot)
})
...
test_that("inspect_cor filtered plots", {
  ...
  expect_doppelganger("Inspect-cor-filtered-starwars-0.5",   inspect_cor(starwars) %>% dplyr::filter(abs(corr) > 0.1) %>% plot)
})



test_that("Grouped df correlation plots work", {
  expect_doppelganger("Inspect-cor-grouped-plot-starwars", starwars %>% dplyr::group_by(gender) %>% inspect_cor %>% plot)
  expect_doppelganger("Inspect-cor-grouped-plot-tech-year", tech %>% dplyr::group_by(year) %>% inspect_cor() %>% plot)
  expect_doppelganger("Inspect-cor-grouped-plot-tech-quarter", tech %>% dplyr::group_by(quarter) %>% inspect_cor() %>% plot)
})
