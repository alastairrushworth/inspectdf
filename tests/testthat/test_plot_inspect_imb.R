...
test_that("inspect_imb plot basic", {
  expect_doppelganger("Inspect-imb-starwars", starwars %>% inspect_imb %>% plot)
  expect_doppelganger("Inspect-imb-storms",   storms %>%   inspect_imb %>% plot)
  expect_doppelganger("Inspect-imb-tech",   tech %>%  inspect_imb %>% plot)  
})

test_that("inspect_imb plot col_palette", {
  expect_doppelganger("Inspect-imb-palette-starwars", starwars %>% inspect_imb %>% plot(col_palette = 1))
  expect_doppelganger("Inspect-imb-palette-storms",   storms %>%   inspect_imb %>% plot(col_palette = 2))
})

test_that("inspect_imb plot paired", {
  expect_doppelganger("Inspect-imb-paired-starwars", starwars %>% inspect_imb(x2) %>% plot)
  expect_doppelganger("Inspect-imb-paired-storms",   storms %>%   inspect_imb(y2) %>% plot)
})

test_that("inspect_imb suppress labels", {
  expect_doppelganger("Inspect-imb-no-lab-starwars", starwars %>% inspect_imb %>% plot(text_labels = FALSE))
  expect_doppelganger("Inspect-imb-no-lab-tech", tech %>% inspect_imb %>% plot(text_labels = FALSE))
